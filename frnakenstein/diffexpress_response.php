...
if (file_exists ("$subdirectories/diffexpress_output/analysis_$analysisname")){
        exit("<h4>Error: Analysis name already in use!</h4>");
}
...
$cdcommand = "mkdir -p $cdoutputpath &&\ncuffdiff -p $procs -o $cdoutputpath -L $controlcondition,$expcondition $cmoutputpath/merged.gtf $bampaths &>> $logfile &&\n";
...
$rcommand = ".libPaths(\"$subdirectories/R/x86_64-pc-linux-gnu-library/3.1\")\n";
...
echo "$bashdir is bashdir!!!!"; 
# Create results path
$resultspath = "http://raven.anr.udel.edu/frnakenstein/results.php?analysis=analysis_$analysisname";

# Combine into one file
$commands .= $singlefilecommand;

# Move folder from temp
$commands .= "mv -f $analysispath $subdirectories/diffexpress_output/".$_SESSION['user_name']."\n";
...
$postmailcommand = 'if [ $? -eq 0 ]; then
	echo "Your DiffExpress run with ID: '.$mytimeid.' completed successfully! You can view and download your data on the visualize page." | mail -s "$(echo -e "fRNAkenstein DiffExpress Successful\nFrom: fRNAkbox <wtreible@raven.anr.udel.edu>\n")" '.$_SESSION['user_email'].'
else
...
$commands .= "\nmv $subdirectories/bash_scripts/r_$mytimeid.R $oldbashdir";
$commands .= "\nmv $bashfile $subdirectories/old_bash_scripts";
